...
class LoadResponse(VisionDataset):
    ...
    def __getitem__(self, index):
        ...
        # # # noise = torch.from_numpy(np.random.normal(0, 0.006, (ax1, ax2, ax3)).astype(np.float32))
        # noise = torch.from_numpy(np.random.uniform(-0.02, 0.02, (ax1, ax2, ax3)).astype(np.float32))
        ...
        # for i in [39,42]:
        #     pos[:, i, [63,67,71,75,79,83,87,92]] = torch.ones_like(pos[:, i, [63,67,71,75,79,83,87,92]]) # C5
        # for j in [133,135]:
        #     pos[:, j, [46,48,51]] = torch.ones_like(pos[:, j, [46,48,51]]) # C7
        # for k in [163,165]:
        #     pos[:, k, [143,147,151,156]] = torch.ones_like(pos[:, k, [143,147,151,156]]) # C10
        # for k1 in [108,112,116]:
        #     pos[:, k1, [132,135]] = torch.ones_like(pos[:, k1, [132,135]])
        # for k2 in [38,41,44]:
        #     pos[:, k2, [139,144]] = torch.ones_like(pos[:, k2, [139,144]])
        # for k3 in [83,87]:
        #     pos[:, k3, [44,48,53]] = torch.ones_like(pos[:, k3, [44,48,53]])
